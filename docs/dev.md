...
* Upload `release.yml` to a GitHub release
* Upload a `checksums.txt` file to a GitHub release with the `release.yml` file checksum
* Generate the changelog of commits included in the release
...
The checksum for `release.yml` will be available in checksums.txt and can be added to the 
release notes as well.

Once the release notes are ready, clicking the `Publish release` button in the GitHub UI will 
...
* [`.goreleaser.yml`](../.goreleaser.yml)
  - We use [goreleaser](https://goreleaser.com/) to carry out the creation of the GitHub release
